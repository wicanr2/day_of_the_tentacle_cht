...
目標是把 CD 版的英文語音換成中文，而且**角色音調貼近英文原音**。做出來的是
兩組——台式中文合成語音與原音克隆——在遊戲中按 Ctrl+T 切換。
...
| 產物 | `monster-tw.sof` 122 MB | `monster-cl.sof` 121 MB |
...
| | 中位 | p10 | p90 | max |
|---|---|---|---|---|
| 台式中文（edge-tts） | 1.60x | 1.29 | 1.92 | 4.11 |
| 原音克隆（F5-TTS） | 0.94x | 0.69 | 1.67 | 8.00（觸上限） |

克隆版**本來就跟原版一樣大聲**——F5-TTS 拿原音當範本時，把響度特性也一起學走了，
所以這一輪對齊對它幾乎是空轉，只在修兩端的離群段。edge-tts 那組則是普遍偏小聲，
確實需要拉。
...
| | 中位差 | p90 | 在 ±3 半音內 |
|---|---|---|---|
| 台式中文（edge-tts） | 1.2 半音 | 3.0 | 90% |
| 原音克隆（F5-TTS） | 2.5 半音 | 5.8 | 52% |

**edge-tts 的音高反而貼得更緊**，這不是意外：它是逐句照該句所屬音高帶挑 pitch，
而 F5-TTS 每個帶只有**一段** reference，克隆出來的音高是那位參考說話者的，
不是這一句原本的。

所以兩組各有所長——edge-tts 音高準、腔調自然；克隆版有角色自己的音色，
但音高散、帶英文母語者的中文腔。這正是把兩組都做出來、在遊戲中可切換的理由。
...
引擎改了五個檔（都在 `patches/scummvm-zhtw.patch` 裡）：
...
`cht_voice_pack=2` 同樣驗過，開的是 `monster-cl.sof`；從 2 連按三次會走
2 → 0 → 1 → 2，設定檔跟著更新。

## A/B 對照音檔

`tools/voice_demo.py` 直接從三份 `.sof` 取同一句拼接，不錄遊戲畫面——錄影會混進
配樂與音效，而且 SDL 的 disk driver 不是即時的（75 秒實際時間會寫出 150 秒音訊），
拿來做 A/B 會失真。沒指定句子時每個音高帶各挑一句（中文 8–25 字），男聲女聲都聽得到。

產物 `dist-all/voice-demo-3way.wav`：10 句 × 3 個版本，101 秒。
