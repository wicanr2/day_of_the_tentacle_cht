#!/usr/bin/env python3
"""用倚天中文系統 (ETEN 3.53) 的原生點陣字烘 chinese_gb16x12.fnt（瘋狂時代版）。
...
字形位置 idx = (lead - 0xA1) * 94 + (trail - 0xA1)（ScummVM ZH_CHN 原生公式），
見 cht_common.py。瘋狂時代是 SCUMM v6，沒有 v1/v2 的空白壓縮問題，
所以碼空間用完整的 0xA1-0xFD，字位數與引擎的 numChar 一致（8178）。
...
from cht_common import NUM_GLYPHS as CAPACITY, bytes_to_idx as font_index
...
def main():
    ...
    # collect_charset.py 產的碼表帶 metadata（generated/sources/count/chars），
    # 攤平成 {字: (lead, trail)}
    if "chars" in table:
